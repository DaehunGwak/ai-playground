...
<div class="main-header">
    <h1>📈 MuQ-MuLan Visualization</h1>
    <p>Explore MuQ-MuLan embeddings in 3D space using dimensionality reduction</p>
</div>
""", unsafe_allow_html=True)
...
def load_tracks_data():
    """Supabase에서 MuQ-MuLan 임베딩이 있는 트랙 데이터를 가져옵니다."""
    supabase = get_supabase_client()
    result = supabase.schema('public').table('tracks') \
        .select('id, title, tags, embeddingMuq') \
        .execute()
    return result.data
...
def create_3d_scatter(coords: np.ndarray, titles: list, tags_short: list, 
    ...
    fig = px.scatter_3d(
        ...
        title=f'🎹 MuQ-MuLan Embeddings - {method_name}',
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            xaxis=dict(backgroundcolor='rgba(30,30,46,0.8)', gridcolor='#4a3d5c'),
            yaxis=dict(backgroundcolor='rgba(30,30,46,0.8)', gridcolor='#4a3d5c'),
            zaxis=dict(backgroundcolor='rgba(30,30,46,0.8)', gridcolor='#4a3d5c'),
        ),
        paper_bgcolor='rgba(0,0,0,0)',
        font=dict(color='#d4a5c9'),
        title_font=dict(color='#ff6b9d', size=18),
    )
...
try:
    # 데이터 로드
    with st.spinner("🔄 Loading MuQ-MuLan embedding data from Supabase..."):
    ...
    # embeddingMuq가 None이 아닌 데이터만 필터링
    tracks_df = tracks_df[tracks_df['embeddingMuq'].notna()].reset_index(drop=True)
    
    if len(tracks_df) == 0:
        st.warning("⚠️ No tracks with MuQ-MuLan embeddings found. Please run the embedding extraction first.")
        st.stop()
    
    # embeddings를 numpy 배열로 변환
    embeddings_list = [parse_embedding(e) for e in tracks_df['embeddingMuq']]
    ...
    with st.sidebar:
        ...
        method = st.selectbox(
            ...
            help="Choose the method for reducing embeddings to 3D"
        )
        ...
        color_scheme = st.selectbox(
            "Point Color",
            ["#ff6b9d", "#4ecdc4", "#ffe66d", "#a855f7", "#f43f5e", "#22d3ee"],
            index=0
        )
        ...
        st.markdown("---")
        st.markdown("### 🎹 MuQ-MuLan Info")
        st.caption("MuQ-MuLan embeddings capture music-language relationships with multilingual support (English & Chinese)")
    ...
    with st.expander("View all tracks with MuQ-MuLan embeddings", expanded=False):
    ...
    with col1:
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"muq_embeddings_{method.lower().replace(' ', '_')}.csv",
            mime="text/csv"
        )
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=json_data,
            file_name=f"muq_embeddings_{method.lower().replace(' ', '_')}.json",
...
st.markdown("---")
st.markdown(
    "<div style='text-align: center; color: #666;'>Powered by 🎹 MuQ-MuLan, scikit-learn, UMAP & Plotly</div>",
    unsafe_allow_html=True
)
